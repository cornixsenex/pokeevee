...
static bool8 DisplayPartyPokemonDataForMoveTutorOrEvolutionItem(u8 slot)
{
    ...
    else
    {
		...
		//Cornix Custom check Live Bait Tool
		if (item == ITEM_LIVE_BAIT_TOOLS)
		{
			if (PokemonCanBeUsedAsLiveBait(currentPokemon))
				return FALSE;
			else
			{
...
void Task_LiveBaitTools(u8 taskId)
{
	static const u8 askLiveBaitText[] =  _("Would you like to turn {STR_VAR_1}\ninto Live Bait?");
	static const u8 doneLiveBaitText[] = _("You turned {STR_VAR_1} into Live Bait.\nBye Bye {STR_VAR_1}!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
	...
	case 3:
		PlaySE(SE_USE_ITEM);
        StringExpandPlaceholders(gStringVar4, doneLiveBaitText);
        DisplayPartyMenuMessage(gStringVar4, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
        break;
...
//All this function does is calls the task, compare AbilityCapsule
void ItemUseCB_LiveBaitTools(u8 taskId, TaskFunc task)
{
    s16 *data = gTasks[taskId].data;
	tState = 0;
	gTasks[taskId].func = Task_LiveBaitTools;
}
